classdef casadi_bat < matlab.System
    ...
    methods(Access = protected)
        ...
        function [ocp, internal_resistance, heating, neg_conc, pos_conc, soh_pct] = stepImpl(obj, current, temperature, soc_init)
            import casadi.*
            ...
            yt = obj.f(obj.y0, 0, [t_non_dim;current;t_min], 0, 0, 0, 0);
            obj.y0 = yt(:, end);
            ...
            pos_conc = temp(5);
            if abs(current) < 1e-9
                internal_resistance = NaN;   % 또는 이전값 유지 로직 사용 가능
            else
                internal_resistance = abs((ocp - v) / current);
            end
            capacity_Ah = temp(6);

    if isempty(obj.Q0) || ~isfinite(obj.Q0) || obj.Q0<=0
        obj.Q0 = max(capacity_Ah, eps);
    end
    soh_pct = 100.0 * capacity_Ah / obj.Q0;

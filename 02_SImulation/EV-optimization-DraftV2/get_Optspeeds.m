function OPTout = get_Optspeeds(Pred_speds, param, degmap, batt)
...
MPCout = get_MPCsolver(param, Pred_speds, v_tot, dist_rel_end, ...
                       lb_v, ub_v, lb_a, ub_a, degmap, dist_init, batt);
...
% -------------------------
pred_P_req  = nan(Nx,1);
pred_I_batt = nan(Nx,1);
pred_Q_Ah   = nan(Nx,1);
pred_SOC    = nan(Nx,1);
pred_trq    = nan(Nx,1);
pred_rpm    = nan(Nx,1);
pred_Q_batt = nan(Nx,1);
pred_Rin    = nan(Nx,1);
pred_Q_loss = nan(Nx,1);
...
for i = 1:Nx
    ...
    pred_P_req(i)  = EVp.P_req;
    pred_I_batt(i) = EVp.I_batt;
    pred_Q_Ah(i)   = EVp.Q_Ah;
    pred_SOC(i)    = EVp.SOC;
    pred_trq(i)    = getfield_safe(EVp,'trq',NaN);
    pred_rpm(i)    = getfield_safe(EVp,'rpm',NaN);
    pred_Q_batt(i) = getfield_safe(EVp,'Q_batt',NaN);
    pred_Rin(i)    = getfield_safe(EVp,'Rin',NaN);
    pred_Q_loss(i) = getfield_safe(EVp,'Q_loss',NaN);

    [EVo, batt_opt] = get_evmodel(opt_v_plot(i), a_opt(i), ...
        opt_dist_rel(i), dist_init, param_i, degmap, batt_opt);

    opt_P_req(i)  = EVo.P_req;
    opt_I_batt(i) = EVo.I_batt;
    opt_Q_Ah(i)   = EVo.Q_Ah;
    opt_SOC(i)    = EVo.SOC;
    opt_trq(i)    = getfield_safe(EVo,'trq',NaN);
    opt_rpm(i)    = getfield_safe(EVo,'rpm',NaN);
    opt_Q_batt(i) = getfield_safe(EVo,'Q_batt',NaN);
    opt_Rin(i)    = getfield_safe(EVo,'Rin',NaN);
    opt_Q_loss(i) = getfield_safe(EVo,'Q_loss',NaN);
end
...
OPTout.pred.P_req  = pred_P_req;
OPTout.pred.I_batt = pred_I_batt;
OPTout.pred.Q_Ah   = pred_Q_Ah;
OPTout.pred.SOC    = pred_SOC;
OPTout.pred.trq    = pred_trq;
OPTout.pred.rpm    = pred_rpm;
OPTout.pred.Q_batt = pred_Q_batt;
OPTout.pred.Rin    = pred_Rin;
OPTout.pred.Q_loss = pred_Q_loss;
OPTout.pred.batt_end = batt_pred;

% prediction (opt)
OPTout.opt.v_plot  = opt_v_plot;    % Nx aligned for plot
OPTout.opt.acc     = a_opt;
OPTout.opt.P_req   = opt_P_req;
OPTout.opt.I_batt  = opt_I_batt;
OPTout.opt.Q_Ah    = opt_Q_Ah;
OPTout.opt.SOC     = opt_SOC;
OPTout.opt.trq     = opt_trq;
OPTout.opt.rpm     = opt_rpm;
OPTout.opt.Q_batt  = opt_Q_batt;
OPTout.opt.Rin     = opt_Rin;
OPTout.opt.Q_loss  = opt_Q_loss;
